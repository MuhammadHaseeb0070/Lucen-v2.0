...
- **Global scope**: `E:/Lucen/Lucen-v2.3 fresh/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/Lucen/Lucen-v2.3 fresh/.claude/notes/`)
...
2. Glob `E:/Lucen/Lucen-v2.3 fresh/.claude/notes/*.md` (if directory exists) — global notes
...
Global (E:/Lucen/Lucen-v2.3 fresh/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/Lucen/Lucen-v2.3 fresh/.claude/notes/` — works in any directory
