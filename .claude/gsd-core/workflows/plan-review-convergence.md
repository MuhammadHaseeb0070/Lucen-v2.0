...
```bash
INIT=$(node "E:/Lucen/Lucen-v2.3 fresh/.claude/gsd-core/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "E:/Lucen/Lucen-v2.3 fresh/.claude/gsd-core/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "E:/Lucen/Lucen-v2.3 fresh/.claude/gsd-core/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
